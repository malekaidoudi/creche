...
exports.getTodayTasks = async (req, res) => {
  try {
    ...
    const appointmentTasks = appointmentsResult.rows.map(apt => ({
      id: `appointment-${apt.enrollment_id}`,
      enrollment_id: apt.enrollment_id,
      type: 'appointment',
      title: `RDV: ${apt.child_first_name} ${apt.child_last_name || ''}`.trim(),
      description: `Rendez-vous avec ${apt.applicant_first_name} ${apt.applicant_last_name || ''}`.trim(),
      time: apt.appointment_time,
      datetime: apt.appointment_date,
      priority: 'high',
      status: 'pending',
      contact: {
        name: `${apt.applicant_first_name} ${apt.applicant_last_name || ''}`.trim(),
        phone: apt.applicant_phone,
        email: apt.applicant_email
      }
    }));
