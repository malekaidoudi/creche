...
const DashboardHome = () => {
  ...
  const statsCards = [
    ...
    {
      title: isRTL ? 'إجمالي الأولياء' : 'Total Parents',
      value: stats?.totalParents || 0,
      icon: Users,
      color: 'text-purple-600',
      bgColor: 'bg-purple-100 dark:bg-purple-900',
      change: '+1',
      changeText: isRTL ? 'هذا الأسبوع' : 'cette semaine'
    }
  ];
  ...
  const quickActions = [
    ...
    },
    {
      ...
      icon: FileText,
      link: '/dashboard/enrollments/pending',
      color: 'text-orange-600',
  ...
  const recentActivities = [
    {
      type: 'enrollment',
      message: isRTL ? 'طلب تسجيل جديد من أحمد محمد' : 'Nouvelle demande d\'inscription de Ahmed Mohamed',
      time: isRTL ? 'منذ 5 دقائق' : 'il y a 5 min',
      icon: ClipboardList,
      color: 'text-blue-600'
    },
    {
      type: 'attendance',
      message: isRTL ? 'سارة أحمد وصلت إلى الحضانة' : 'Sara Ahmed est arrivée à la crèche',
      time: isRTL ? 'منذ 15 دقيقة' : 'il y a 15 min',
      icon: UserCheck,
      color: 'text-green-600'
    },
    {
      type: 'document',
      message: isRTL ? 'وثائق جديدة مرفوعة لطفل محمد علي' : 'Nouveaux documents uploadés pour Mohamed Ali',
      time: isRTL ? 'منذ 30 دقيقة' : 'il y a 30 min',
      icon: FileText,
      color: 'text-orange-600'
    }
  ];
