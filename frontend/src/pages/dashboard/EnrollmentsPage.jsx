...
const EnrollmentsPage = () => {
  ...
  const fetchEnrollments = async () => {
    try {
      ...
      try {
        // Si l'onglet est "rejected", récupérer toutes les inscriptions et filtrer côté client
        const statusParam = activeTab === 'rejected' ? 'all' : (activeTab === 'all' ? 'all' : activeTab);
        const response = await enrollmentsService.getAllEnrollments({
          status: statusParam,
  ...
  const filteredEnrollments = enrollments.filter(enrollment => {
    // Utiliser new_status si disponible, sinon status
    const enrollmentStatus = enrollment.new_status || enrollment.status;

    if (activeTab === 'all') {
      return true; // Afficher toutes les inscriptions
    }
    if (activeTab === 'rejected') {
      return enrollmentStatus === 'rejected_incomplete' || enrollmentStatus === 'rejected_deleted';
    }
    return enrollmentStatus === activeTab;
  });
  ...
  const tabs = [
    ...
    { id: 'approved', label: isRTL ? 'مقبولة' : 'Finalisées', count: allEnrollmentsForCounts.filter(e => (e.new_status || e.status) === 'approved').length },
    {
      id: 'rejected', label: isRTL ? 'مرفوضة' : 'Rejetées', count: allEnrollmentsForCounts.filter(e => {
        ...
        return status === 'rejected_incomplete' || status === 'rejected_deleted';
      }).length
    }
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Filtres - Grille sur smartphone, onglets sur tablette/desktop */}
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-3 md:p-0 md:bg-transparent md:shadow-none">
        <h3 className="text-sm font-medium text-gray-900 dark:text-white mb-3 md:hidden">
          {isRTL ? 'تصفية حسب الحالة' : 'Filtrer par statut'}
        </h3>
                ...
                <CardContent className="p-3 sm:p-4 md:p-6">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-0">
                    <div className="flex-1">
                      <div className="flex items-center space-x-4 rtl:space-x-reverse mb-3">
                        <div className="flex items-center space-x-2 rtl:space-x-reverse">
                          {getStatusIcon(enrollment.new_status || enrollment.status)}
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(enrollment.new_status || enrollment.status)}`}>
                            {getStatusLabel(enrollment.new_status || enrollment.status)}
                          </span>
                        </div>
                    ...
                    <Button
                      ...
                      className="w-full md:w-auto text-xs sm:text-sm px-3 py-2"
                    >
                      <Eye className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1 rtl:mr-0 rtl:ml-1" />
          ...
          >
            <div className="p-6">
              ...
              <div className="space-y-4">
                ...
                {/* Date de rendez-vous */}
                <DatePicker
                  label={isRTL ? 'تاريخ الموعد (اختياري)' : 'Date de rendez-vous (optionnel)'}
                  title={isRTL ? 'تاريخ الموعد' : 'Date de rendez-vous'}
                  value={approvalData.appointment_date}
                  onChange={(value) => setApprovalData(prev => ({ ...prev, appointment_date: value }))}
                />
